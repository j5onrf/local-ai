...
class InlineSpinner:
    ...
    def __init__(self, chars: str = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏") -> None:
        self.chars, self.active, self.thread, self.message, self.start_time = chars, False, None, "Thinking...", 0.0
...
def get_key() -> str:
    ...
    if sys.stdin.isatty():
        try: return _read_fd(sys.stdin.fileno())
        except Exception: pass
    try:
        with open("/dev/tty", "r") as f: return _read_fd(f.fileno())
    except Exception: pass
    try: return os.read(sys.stdin.fileno(), 1).decode("utf-8", errors="ignore")
...
def get_local_model_name() -> str:
    ...
    except Exception: return "local-model"
...
def draw_session_box(
    ...
    if box_style == 5:
        ...
        panel = Panel(
            table, title=Text(title_text, style=title_style), title_align="left",
            border_style=border_col, box=box_type, expand=False, subtitle="[dim]Ctrl+C to exit[/dim]", subtitle_align="right"
        )
...
def show_help() -> None:
    ...
    header = Text.assemble(
        ("  Shortcuts: ", "dim"), ("Esc", "bold yellow"), (": bypass  ", "dim"),
    ...
    cmds = [
        ("/h", "Help menu"), ("/box [1-5]", "Box style preset"), ("/t [N|show|hide]", "Set reasoning budget or show/hide"),
