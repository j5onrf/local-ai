...
def end(actual_out_tokens: int = None, is_local: bool = False) -> None:
    ...
    if is_local and think_toks > 0:
        msg = f"\033[90m [ think: {think_toks} | ans: {ans_toks} | {tot_toks} tokens | {elapsed:.1f}s @ {tps:.1f} t/s ]\033[0m\n"
    else:
        msg = f"\033[90m [ {tot_toks} tokens | {elapsed:.2f}s | {tps:.2f} t/s ]\033[0m\n"
