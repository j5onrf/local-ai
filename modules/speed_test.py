...
def start() -> None:
    ...
    global _state
    _state = {"start": time.time(), "t_start": None, "t_end": None, "t_chars": 0, "a_chars": 0, "in_think": False}
...
def count_token(content: str, is_thinking: bool = False) -> None:
    ...
    if not content or _state["start"] is None: return
    now = time.time()
    if is_thinking:
...
def end(actual_out_tokens: int = None, is_local: bool = False, resolved_model: str = None, active_model: str = None) -> None:
    """Calculates, prints token statistics, and resets state."""
    if _state["start"] is None: return
    elapsed = max(0.001, time.time() - _state["start"])
    if _state["in_think"] and not _state["t_end"]: _state["t_end"] = time.time()

    tot_chars = _state["t_chars"] + _state["a_chars"]
    tot_toks = actual_out_tokens if (actual_out_tokens and actual_out_tokens > 0) else max(1, round(tot_chars / 4.0)) if tot_chars > 0 else 0
    think_toks = round((_state["t_chars"] / tot_chars) * tot_toks) if tot_chars > 0 and _state["t_chars"] > 0 else 0
    ans_toks = max(0, tot_toks - think_toks)

    tps = tot_toks / elapsed

    model_line = ""
    if active_model == "openrouter/free" and resolved_model and resolved_model != "openrouter/free":
    ...
    model_line = ""
    if model and model != "openrouter/free" and "openrouter/free" in str(os.environ.get("OPENROUTER_MODEL", "")).lower():
        model_line = f"\033[90m [ model: {model} ]\033[0m\n"
