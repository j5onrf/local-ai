...
*   **Offline Compilation:** Runs entirely locally, offline, and with zero external dependencies.
...
### ⚙️ Human-in-the-Loop Graph Queries

When the AI agent suggests a structural query, execute the intent trigger inside your prompt. The agent will prompt you for authorization before executing the tool and injecting the target context:
...
*   `find symbol <pattern>`: Performs a rapid local SQL search on codebase symbols.
